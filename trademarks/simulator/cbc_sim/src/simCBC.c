...
void kissSimComputerUpdateDigitals()
{
	...
	for(i=0;i<8;i++) {
		d[i]=!(!kiss_get_key(i+48));//if a number key is pressed set that d to 1 otherwise set to 0
	...
	for (i=0;i<11;i++){
		...
		if(kiss_get_mouse_button(0) && x>=x0 && x <=x1 && (y>= (y0 + i*yOffset)) && (y<= (y1 + i*yOffset))){
			if(i<8)_bob.analogs[i]=(x-x0)*8;
				...
				if(i==8)_bob.acc_x=(x-x0)*32;
				if(i==9)_bob.acc_y=(x-x0)*32;
				if(i==10)_bob.acc_z=(x-x0)*32;
			}
		}
	}
}
...
int kissSimComputerInstructions(int i)
{
	...
	i++;
	NOBOLD g_printString("USE THE 0-7 KEYS TO TOGGLE DIGITALS ON AND OFF.",5, 5+i*15,TEAL,1.0);
	i++;
