...
void kissSimRobotDraw()
{
	...
	if(robot.pen){data[12]='N';data[13]=' ';}
	...
	for(i=0; i<robot.pixelCount;i++) graphics_pixel(robot.pixel[i].x,robot.pixel[i].y,0,0,0);
...
void kissSimRobotInit()
{
	...
	robot.lenc=robot.renc=robot.pen=robot.pixelCount=0;
